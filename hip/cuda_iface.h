...
#define PS_SYMCPY(sym, src, count) \
    hipMemcpyToSymbol(HIP_SYMBOL(sym), src, count)

#define PS_SYMCPY_ASYNC(sym, src, count, stream) \
    hipMemcpyToSymbolAsync(HIP_SYMBOL(sym), src, count, 0, hipMemcpyHostToDevice, stream)

#define PS_SYMCPY_FROM_ASYNC(dst, sym, count, stream) \
    hipMemcpyFromSymbolAsync(dst, HIP_SYMBOL(sym), count, 0, hipMemcpyDeviceToHost, stream)

#elif !defined(PS_DRIVER_API)
/* ------------------------------------------------------------------ */
