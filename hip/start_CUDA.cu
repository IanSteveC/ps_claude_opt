...
//      fprintf(stderr, "Block dim: %d\n", CUDA_BLOCK_DIM);
  ...
  res = PS_SYMCPY(CUDA_Weight, weight, (ndata + 3 + 1) * sizeof(double)); //, hipMemcpyHostToDevice);
  //res = PS_SYMCPY(CUDA_Weight, &pWeight, sizeof(pWeight));
  
  res = PS_SYMCPY(CUDA_ee, ee, 3 * (MAX_N_OBS + 1) * sizeof(double)); //, hipMemcpyHostToDevice);
  res = PS_SYMCPY(CUDA_ee0, ee0, 3 * (MAX_N_OBS + 1) * sizeof(double)); //, hipMemcpyHostToDevice);
		...
		int ndata, int *ia, int *ia_par, int *new_conw, double *cg_first, double *sig, double *sigr2, int Numfac, double *brightness)
{
  ...
  err = hipMalloc(&pco, (size_t)(CUDA_Grid_dim_precalc) * (lmfit + 1) * (lmfit + 2) * sizeof(double));
  err = hipMalloc(&pdytemp, (size_t)(CUDA_Grid_dim_precalc + 1) * (max_lp + 1) * DYT_STRIDE * sizeof(double));
	      ...
	      double *cg_first, MFILE &mf, double escl, double *sig, double *sigr2, int Numfac, double *brightness)
{
  ...
  err = hipMalloc(&pco, (size_t)CUDA_grid_dim * (lmfit + 1) * (lmfit + 1) * sizeof(double));
  // dytemp/ytemp are per-curve scratch, sized by the longest curve rather than ndata
  err = hipMalloc(&pdytemp, (size_t)(CUDA_grid_dim + 1) * (max_lp + 1) * DYT_STRIDE * sizeof(double));
  err = hipMalloc(&pytemp, (size_t)CUDA_grid_dim * (max_lp + 1) * sizeof(double));
  if(err != hipSuccess || pco == NULL || pdytemp == NULL || pytemp == NULL)
    {
