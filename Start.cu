...
/* All arithmetic reads the full-precision DsphT table on every arch. A
   ...
   data-center speed trick (~25% on V100); it was removed because it cost
   exact pole (lambda/beta) agreement with the CPU reference (full-precision
   path validated: 10-input suite, zero pole flips, V100). */
